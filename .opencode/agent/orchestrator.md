...
7. Après toute modification de calcul financier : les **10 cas types du pied de facture** (avec/sans retenue, avec/sans droit de timbre) passent au centime près, le barème du timbre se lit depuis les paramètres (jamais en dur).
...
- Points ouverts (§16 — valeurs du barème du timbre, TAP, longueurs NIF/NIS, template GITRA) : **poser la question, ne pas trancher seul** ; ces valeurs vivent en table de paramétrage, jamais en dur. (Mécanique tranchée le 09/08/2026 : timbre = espèces uniquement, seuil 1 M DA paramétré ; intérêts moratoires = montant saisi sur ND, pas de taux.)
...
- Hors périmètre MVP (interdit sans demande) : déclarations mensuelles et ST, révision de prix, rapport mensuel GITRA (**bloqué** par l'absence de template, §5.3), créances, cautions, retenues de garantie, sous-traitance, dashboards, rapports personnalisables.
