...
import type { DonneesPdfBl } from './types'
...
const formaterQuantite = (quantiteMilliemes: number): string =>
  quantiteMilliemes > 1000 ? String(quantiteMilliemes / 1000) : String(quantiteMilliemes)

const lignesTableau = (donnees: DonneesPdfBl): Content[][] => {
  const enTetes: Content[] = ['Désignation', 'Unité', 'Qté']
  const corps: Content[][] = donnees.lignes.map((ligne) => [
    ligne.designation,
    ligne.unite,
    formaterQuantite(ligne.quantiteMilliemes),
  ])
  return [enTetes, ...corps]
}

const mentionsLegales = (entreprise: DonneesPdfBl['entreprise']): Content[] => [
...
export const construireGabaritBl = (donnees: DonneesPdfBl): TDocumentDefinitions => {
  ...
  const blocTitre: Content[] = [
    { text: 'BON DE LIVRAISON', style: 'titreDocument' },
    { text: `N° ${donnees.numero}`, style: 'numeroDocument' },
    { text: `Date : ${formaterDate(donnees.dateLivraison)}`, style: 'infoDocument' },
  ]
  ...
  const poids: Content[] = []
  if (donnees.poidsPeseeKg !== null) {
    poids.push({ text: `Poids : ${String(donnees.poidsPeseeKg)} kg`, style: 'poidsPesee' })
  }
  ...
  return {
    ...
    styles: {
      ...
      poidsPesee: { fontSize: 10, bold: true, alignment: 'right', margin: [0, 3, 0, 0] },
